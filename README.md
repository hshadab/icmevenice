...
2. **ICME enforces your company's rules.** Before any money moves, ICME checks the agent's decision against your procurement policy: Is the vendor on the approved list? Is the amount under the spending limit? Is their SOC2 certification current? If any rule is violated, the transaction is blocked — no exceptions.
...
3. **ICME checks the rules** — the agent tells ICME: "I want to pay $8,400 to SecureCompute." ICME checks this against your compiled policy. Is the vendor approved? Is SOC2 valid? Is the amount under $25K? It returns SAT (all rules pass) or UNSAT (something's wrong).
...
- **ICME** — [docs.icme.io](https://docs.icme.io)
  ...
  - `POST /v1/checkIt` — Check an action against policy (1 credit). Returns `{ check_id, result: "SAT"|"UNSAT", detail, extracted, verification_time_ms }`
